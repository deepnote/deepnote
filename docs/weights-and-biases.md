...
If the notebook/data app is public, the Wandb's token shouldn't be shared publicly. In Deepnote, it can be solved with the `Integrations` section in the project's **Settings** panel (open **Settings** from the top-right), when selecting the option `Create new integration`, then by clicking on `Environment variables` to create a variable and insert the token generated in the W&B account.

In the image below, the example workspace of a specific project is shown. The graph displays the progression of metrics such as test accuracy or loss. The outputs and logs are also available in the wandb folder within the project's **Files** section (open the **Settings** panel from the top-right).
