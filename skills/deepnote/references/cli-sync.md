...
prompts come one at a time after every other project has finished, and an override is applied only
after the project is re-read and re-exported; with `--concurrency 1` they come inline. In `-o json`, the synced-project entries are sorted by path,
