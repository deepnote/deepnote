...
| `deepnote sync [dir]`                 | Mirror Deepnote Cloud projects into a local directory: pull every project into `<folder path>/<project name>/` (one `.deepnote` per notebook), following the workspace folder tree. State in `.deepnote-sync.json`; conflicts prompt (or `--on-conflict skip\|override`). Push is deferred. |
