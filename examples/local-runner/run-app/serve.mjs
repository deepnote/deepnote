...
// from the browser — runs, run history, scheduling, and the pipeline itself are all `fetch` calls
// made in the page. This exists only so ./pipelines.js resolves to the built bundle without a copy
// step; publish the folder as static files and nothing here runs.
...
const routes = {
  ...
  '/pipelines.js': [
    join(here, '..', '..', '..', 'packages', 'pipelines', 'dist', 'browser.iife.js'),
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    const missingBundle = route[0].endsWith('browser.iife.js')
    res
      .writeHead(500)
      .end(
        missingBundle
          ? 'pipelines bundle not built. Run: pnpm --filter @deepnote/pipelines build'
...
server.listen(0, '127.0.0.1', () => {
  ...
  console.log(`\n  Deepnote pipelines · run app (static preview) → http://127.0.0.1:${port}`)
