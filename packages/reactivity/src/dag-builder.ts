...
import type { BlockDependencyDag, DagEdge, DagNode } from './types'
 ...
 */
export function buildDagFromBlocks(blocks: BlockContentDepsWithOrder[]): BlockDependencyDag {
  const edges: DagEdge[] = []
  ...
  const nodes = blocksWithUsedImportedModules.reduce<Record<string, DagNode>>((acc, block) => {
    acc[block.id] = {
      ...
      packageFromImports: block.packageFromImports ?? {},
