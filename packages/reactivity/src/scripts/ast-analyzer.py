...
class VariableVisitor(ast.NodeVisitor):
    def __init__(self) -> None:
        ...
        self.package_from_imports = {}  # package -> [names] for "from X import Y"
    ...
    for block in blocks:
        try:
            ...
            if block.get("type") == "code" or block.get("type") is None:
                block_defined, block_used, block_imported, block_packages, block_pkg_aliases, block_from_imports = get_defined_used_variables(
                    block
                )
                ...
                block_from_imports_sorted = {k: sorted(v) for k, v in block_from_imports.items()}
                analysis.append(
                    {
                        "id": block["id"],
                        ...
                        "packageFromImports": block_from_imports_sorted,
                        "linesOfCode": loc,
                    }
                )
            elif block["type"] == "sql":
