...
reading the pipeline out of a `.deepnote` file — see [`@deepnote/pipelines`](../pipelines). That
package needs neither this one nor a kernel: every step is an HTTP call, so a pipeline also runs in a
browser page.
