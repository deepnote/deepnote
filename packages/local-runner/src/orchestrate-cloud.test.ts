...
import { orchestrateInCloud } from './orchestrate-cloud'
import type { OrchestrationEvent } from './orchestrate-core'
...
describe('orchestrateInCloud', () => {
  ...
  it('sends the viewer token to the configured API origin', async () => {
    await orchestrateInCloud(async ({ run }) => run({ id: 'a', notebookId: 'nb-a' }), {
      token: TOKEN,
  ...
  it('defaults to the Deepnote API origin', async () => {
    await orchestrateInCloud(async ({ run }) => run({ id: 'a', notebookId: 'nb-a' }), { token: TOKEN })
  ...
  it('refuses an input value the runs API has no representation for', async () => {
    await expect(
      orchestrateInCloud(async ({ run }) => run({ id: 'a', notebookId: 'nb-a', inputs: { portfolio: { total: 1 } } }), {
        token: TOKEN,
      })
    ).rejects.toThrow('Deepnote inputs accept a string, boolean, or array of strings')
  })

  it('rejects a step that asks for a local kernel, which a browser does not have', async () => {
    await expect(
      orchestrateInCloud(async ({ run }) => run({ id: 'a', notebookId: 'nb-a', target: 'local' }), { token: TOKEN })
    ).rejects.toThrow('targets a local kernel, which a browser does not have')
  })

  it('rejects a step with no notebookId, since it cannot create one', async () => {
    await expect(orchestrateInCloud(async ({ run }) => run({ id: 'a' }), { token: TOKEN })).rejects.toThrow(
      'has no notebookId'
    )
  })
