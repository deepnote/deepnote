...
describe('edit-integration mariadb', () => {
  let tempDir: string
...
`
  ...
  beforeEach(async () => {
    vi.clearAllMocks()
    vi.restoreAllMocks()
    tempDir = await mkdtemp(join(tmpdir(), 'edit-integration-mariadb-test-'))
  })
  ...
  it('edits mariadb integration keeping all defaults', async () => {
