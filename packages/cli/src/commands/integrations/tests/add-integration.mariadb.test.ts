...
describe('add-integration mariadb', () => {
  let tempDir: string

  beforeEach(async () => {
    vi.clearAllMocks()
    vi.restoreAllMocks()
    tempDir = await mkdtemp(join(tmpdir(), 'add-integration-mariadb-test-'))
  })
  ...
  it('creates mariadb integration and stores password as env ref', async () => {
  ...
  it('creates mariadb integration with SSH tunnel enabled', async () => {
