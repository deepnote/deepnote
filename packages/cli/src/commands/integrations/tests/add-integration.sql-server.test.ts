...
describe('add-integration sql-server', () => {
  let tempDir: string
  ...
  async function fillBaseFields(
    ...
    await screen.next()
    expect(screen.getScreen()).toContain('Port: (1433)')
  ...
  it('creates sql-server integration and stores password as env ref', async () => {
  ...
  it('creates sql-server integration with SSH tunnel enabled', async () => {
