...
describe('edit-integration sql-server', () => {
  let tempDir: string
...
`
  ...
  beforeEach(async () => {
    vi.clearAllMocks()
    vi.restoreAllMocks()
    tempDir = await mkdtemp(join(tmpdir(), 'edit-integration-sql-server-test-'))
  })
  ...
  it('edits sql-server integration keeping all defaults', async () => {
